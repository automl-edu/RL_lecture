...
\alert{Remark:} $\hat{V}^\pi(s'; \vect{w})$ is the "old" approximation of the future state value and $\hat{V}(s'; \vect{w})$ is the objective for the update. Therefore, $\hat{V}^\pi(s'; \vect{w})$ is treated as a constant in the derivative.
\end{frame}
%-----------------------------------------------------------------------
%----------------------------------------------------------------------
\begin{frame}[c]{Temporal Difference (TD(0)) Learning with Value
